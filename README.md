...
```bash
go install github.com/Nemial/bx-pack/cmd/bx-pack@latest
```

## Быстрый старт

1. Создайте базовую структуру модуля в новой директории:
   ```bash
...
Создает файл `.bxpack.yml` со стандартными настройками в текущей директории. Если файл уже
существует, команда завершится с ошибкой.
...
bx-pack version bump minor   # 1.0.0 → 1.1.0
bx-pack version bump major   # 1.0.0 → 2.0.0
```

**Вывод:**

```
Версия успешно обновлена
Новая версия: 1.0.1
```
...
- [`strict.yml`](examples/strict.yml) — Конфигурация для строгого соблюдения стандартов
  маркетплейса.
- [`extended_exclude.yml`](examples/extended_exclude.yml) — Пример с расширенным списком
...
- `command` (string): Исполненная команда (`init`, `validate`, `build`, `version show`).
...
- `version` (string): Текущая версия модуля (только для команды `version show`).
...
- Готовый GitHub Action / GitLab CI шаблон для `validate` и `build`.
- Homebrew tap для установки без Go-тулчейна.
...
2. **Кастомная разработка**: Добавление специфических валидаторов под нужды вашей компании.
